...
asian_countries=[c.get for c in countries_data if c.get("region")=="Asia"]
print(asian_countries)

asian_countries_filter=filter
...
sort_countries=sorted(countries_data,key=lambda c:c.get("population"),reverse=True)
print(sort_countries)

maped_sortry=[]
# display all unique region

all_region={c.get("region")for c in countries_data}
print(all_region)
#independent country names

independent_country=[c.get("country_name")for c in countries_data if c.get("is_independent")]
